...
@Composable private fun SettingsContent(settings: Settings) {
    ...
    var accent by remember { mutableStateOf(settings.getString("accent", "blue")) }
    var pagesLayout by remember { mutableStateOf(settings.getString("pages_layout", "card")) }
    ...
    Column(Modifier.fillMaxSize().background(MaterialTheme.colorScheme.background).verticalScroll(androidx.compose.foundation.rememberScrollState()).padding(horizontal = 24.dp, vertical = 20.dp).statusBarsPadding(), verticalArrangement = Arrangement.spacedBy(4.dp)) {
        ...
        Row(horizontalArrangement = Arrangement.spacedBy(8.dp), modifier = Modifier.padding(vertical = 8.dp)) { (listOf("system") + AccentOptions.map { it.id }).forEach { value -> FilterChip(selected = accent == value, onClick = { accent = value; settings.set("accent", value) }, label = { Text(value) }) } }
        Text("pages layout", style = MaterialTheme.typography.bodyLarge)
        Row(horizontalArrangement = Arrangement.spacedBy(8.dp), modifier = Modifier.padding(vertical = 8.dp)) { listOf("card", "row").forEach { value -> FilterChip(selected = pagesLayout == value, onClick = { pagesLayout = value; settings.set("pages_layout", value) }, label = { Text(value) }) } }
